...
function Level:init()
    ...
    -- flag to check if there was a collision
    ...
    -- shows player before being launched and its trajectory arrow
    ...
    -- spawn an enemy alien to try and destroy
...
function Level:update(dt)
    ...
    if love.keyboard.wasPressed('space') then
        if not self.hadCollision and not self.launchMarker.haveSplit then
    ...
    if self.launchMarker.launched then
        ...
        -- if the player has already split then we get the velocity of the fastest moving alien
        if self.launchMarker.haveSplit then
            local topXvel, topYvel = self.launchMarker.topAlien.body:getLinearVelocity()
            local botXvel, botYvel = self.launchMarker.botAlien.body:getLinearVelocity()
            if (math.abs(topXvel) + math.abs(topYvel)) > (math.abs(xVel) + math.abs(yVel)) then
                xVel, yVel = topXvel, topYvel
            end
            if (math.abs(botXvel) + math.abs(botYvel)) > (math.abs(xVel) + math.abs(yVel)) then
                xVel, yVel = botXvel, botYvel
            end
        end
